...
function hasBoxOfResentment() {
  return hasPixelInArea([0xde9473, 2], 950, 30);
}
...
async function warp() {
  await Keyboard.send(KeyCode.ALT, KeyCode.ZERO);
  await Keyboard.send(KeyCode.ALT, KeyCode.ZERO);
  await Keyboard.send(KeyCode.ALT, KeyCode.ZERO);
  await Keyboard.send(KeyCode.ALT, KeyCode.ZERO);
  await Keyboard.send(KeyCode.ALT, KeyCode.ZERO);
...
async function searchAndDestroy(count, foundAnything = false) {
  ...
  // const coordinates = await searchPixel(0xe08040, 8);
  const coordinates = await searchPixel(0x00ff80, 8);
  if (coordinates) {
    const { x, y } = coordinates;

    // if (count > 17) {
    //   // Randomness in behavior, try to move closer
    //   const { percentDistance } = getDifferenceFromCenter(x, y);

    //   if (percentDistance > 0.3) {
    //     console.log("Moving...");
    //     const moveToX = Math.round(x / 2);
    //     const moveToY = Math.round(y / 2);
    //     await MoveCursor(-moveToX, -moveToY);
    //     await sleep(10);
...
async function printScreen() {
  await Keyboard.send(KeyCode.PRINT_SCREEN);
}

function getDifferenceFromCenter(x, y) {
  const halfOfWindowX = windowPos.width / 2;
  const halfOfWindowY = windowPos.height / 2;
  const diagonalLength = Math.sqrt(halfOfWindowX ** 2 + halfOfWindowY ** 2);

  const centerX = windowPos.x + halfOfWindowX;
  const centerY = windowPos.y + halfOfWindowY;
  ...
  return {
    ...
    y: deltaY,
    distance,
    percentX: Math.abs(deltaX) / windowPos.width,
    percentY: Math.abs(deltaY) / windowPos.height,
    percentDistance: distance / diagonalLength,
  };
}
...
async function checkIfDead() {
  return await ahk.pixelSearch(380, 524, 659, 569, 0xffffff, 1);
}
...
async function storeBoxOfResentment() {
  ...
  await doUntilConditionIsTrue({
    condition: () => hasPixelInArea([0xffffff, 1], 24, 88),
  ...
  await doUntilConditionIsTrue({
    condition: () => hasPixelInArea([0xffffff, 1], 276, 107),
...
async function storeETCs() {
  ...
  await MoveCursor(windowPos.x + 56, windowPos.y + 95);
...
async function main() {
  ...
  scanArea = {
    x1: Math.floor(windowPos.x + 0 * windowPos.width),
  ...
  const RESENTMENT_INTERVAL = 0;
  const ETC_INTERVAL = 100;
  let x = 0;
  let mobFindCount = 0;
  ...
  while (x < LOOP_SIZE) {
    ...
    if (await searchAndDestroy(20)) {
      ...
      if (mobFindCount >= 10) {
        await doStuffForXMs([bulkSell], 200);
        mobFindCount = 0;
      } else {
        if (mobFindCount % 3 === 0) {
          log("Trying to open gifts then bulk sell...");
          await doStuffForXMs(
            [openGiftBox, openGiftBox, openGiftBox, bulkSell],
            200
          );
        } else {
          log("Trying to open gifts...");
          await doStuffForXMs([openGiftBox], 200);
        }
      }
      await sleep(50);
    }
    ...
    if (
      RESENTMENT_INTERVAL &&
      x % RESENTMENT_INTERVAL === RESENTMENT_INTERVAL - 1
    ) {
      log("Storing Box of Resentments");
      await storeBoxOfResentment();
    }
    ...
    await doStuffForXMs(
      [useGreenPotions, useWhitePotions, useBluePotions, openGiftBox],
      100
    );
    await sleep(60);
    x++;
