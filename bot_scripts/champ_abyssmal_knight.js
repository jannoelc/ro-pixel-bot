...
async function searchAndDestroy(count, foundAnything = false) {
  ...
  const coordinates = await searchPixel(0x0f0810, 8);
  if (coordinates) {
    const { x, y } = coordinates;
...
async function hunt() {
  ...
  while (failureCount < 20) {
    if (await searchAndDestroy(1)) {
      await searchAndDestroy(15);
      break;
    }
...
// @alootsave +603 +644 +12048 +582 +1035 +1036 +7444 +2114 +7448
