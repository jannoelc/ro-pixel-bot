...
async function warp() {
  await Keyboard.send(KeyCode.ALT, KeyCode.SEVEN);
  await Keyboard.send(KeyCode.ALT, KeyCode.SEVEN);
  await Keyboard.send(KeyCode.ALT, KeyCode.SEVEN);
  await Keyboard.send(KeyCode.ALT, KeyCode.SEVEN);
  await Keyboard.send(KeyCode.ALT, KeyCode.SEVEN);
...
async function main() {
  ...
  while (x < LOOP_SIZE) {
    ...
    if (await searchAndDestroy(20)) {
      ...
      if (mobFindCount >= 10) {
        ...
        if (mobFindCount % 3 === 0) {
          ...
          await doStuffForXMs(
            [useGreenPotions, useOrange, openGiftBox, useOrange, bulkSell],
            1000
          );
        } else {
          log("Trying to open gifts...");
          await doStuffForXMs([openGiftBox, useOrange, useGreenPotions], 1000);
        }
      }
      await sleep(100);
    }
    ...
    await doStuffForXMs([useWhitePotions, useBluePotions, openGiftBox], 100);
    await sleep(60);
    x++;
  }
